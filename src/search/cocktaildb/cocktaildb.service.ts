...
export class CocktailDbService {
            ...
            default:
                return forkJoin(
                    [this.searchByCocktailName(str),
                        this.searchByIngredientName(str)]).pipe(
                            map((arr) => {
    ...
    private searchByCocktailName(str: string): Observable<AxiosResponse<CocktailResults>> {
        return this.httpService.get(process.env.COCKTAIL_DB_URI + search_path, {
            params: {"s": str}
        })
    }

    private searchByIngredientName(str: string): Observable<AxiosResponse<CocktailResults>> {
        return this.httpService.get(process.env.COCKTAIL_DB_URI + search_path, {
            params: {"i": str}
        })
    }
